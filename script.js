...
function setTime() {
  document.querySelector("#local-time").textContent = new Intl.DateTimeFormat("en-GB", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(new Date());
}
...
const loader = gsap.timeline({ onComplete: () => { document.querySelector(".preloader").remove(); document.body.style.overflow = ""; ScrollTrigger.refresh(); } });
...
gsap.to(".marquee-a", { xPercent: -50, duration: 28, ease: "none", repeat: -1 });
gsap.fromTo(".marquee-b", { xPercent: -50 }, { xPercent: 0, duration: 24, ease: "none", repeat: -1 });
...
gsap.from(".breadcrumb", { opacity: 0, y: -10, duration: .5, delay: .9 });
...
if (!prefersReduced && window.innerWidth > 760) {
  ...
  Draggable.create(rail, { type: "x", bounds: () => ({ minX: maxX(), maxX: 0 }), inertia: inertiaAvailable, edgeResistance: .75, onDrag: updateCards, onThrowUpdate: updateCards });
  function updateCards() { const x = gsap.getProperty(rail, "x"); document.querySelectorAll(".feature-card").forEach((card, i) => { const center = x + i * (card.offsetWidth + 17) + card.offsetWidth/2; const distance = Math.abs(center-window.innerWidth/2)/(window.innerWidth/2); gsap.to(card, { scale: Math.max(.88, 1-distance*.13), opacity: Math.max(.45, 1-distance*.55), duration: .15, overwrite: true }); }); }
...
document.querySelector(".subscribe form").addEventListener("submit", (e) => { e.preventDefault(); const button = e.currentTarget.querySelector("button"); button.innerHTML = "Subscribed ✓"; gsap.fromTo(button, { color: "var(--coral)" }, { color: "var(--lime)", duration: .4 }); });
...
if (!prefersReduced && window.THREE && window.innerWidth > 760) {
  const mount = document.querySelector("#three-canvas"), scene = new THREE.Scene(), camera = new THREE.PerspectiveCamera(42, mount.clientWidth/mount.clientHeight, .1, 100), renderer = new THREE.WebGLRenderer({ alpha:true, antialias:true });
